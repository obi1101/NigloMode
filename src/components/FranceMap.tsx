  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="relative select-none" style={{ maxWidth: 680, margin: "0 auto", width: "100%" }}>
                ...
                geographies.map((geo) => {
                  ...
                  return (
                    <Geography
                      ...
                      style={{
                        default: {
                          fill: isActive ? "#F5C218" : hasDonnees ? "#2D6A4F" : "#3d6b47",
                          stroke: "#1A2562",
                          strokeWidth: 0.4,
                          outline: "none",
                          cursor: "pointer",
                        ...
                        hover: {
                          fill: isActive ? "#F5C218" : "#5aad76",
                          stroke: "#1A2562",
                          strokeWidth: 0.6,
                          outline: "none",
                          cursor: "pointer",
                        },
                        pressed: { fill: "#F5C218", outline: "none" },
                      }}
                    />
                  );
                })
        ...
        {tooltip && (
          <div
            className="absolute pointer-events-none px-3 py-1.5 rounded-lg text-xs font-semibold shadow-lg z-10"
            style={{ left: tooltip.x + 12, top: tooltip.y - 10, backgroundColor: "#1A2562", color: "#F5C218", whiteSpace: "nowrap" }}
          >
            🦔 {tooltip.nom}
          </div>
        )}
        <p className="text-center text-xs opacity-40 mt-2" style={{ color: "var(--navy)" }}>
        ...
        <div className="max-w-3xl mx-auto w-full flex flex-col gap-5">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className="text-sm opacity-50">{annoncesFiltrees.length} annonce{annoncesFiltrees.length > 1 ? "s" : ""} dans ce périmètre</p>
            </div>
            <button onClick={() => setDeptActif(null)} className="text-xs opacity-40 hover:opacity-70">✕ Changer de département</button>
          </div>

          {/* Filtres */}
          <div className="rounded-xl p-5 flex flex-wrap gap-4 items-end"
            style={{ backgroundColor: "#f0ede4", border: "1px solid #e0d8c8" }}>

            {/* Rayon */}
            <div className="flex flex-col gap-1">
              <label className="text-xs font-bold uppercase tracking-widest" style={{ color: "#5a4a30" }}>Rayon</label>
              <div className="flex gap-1.5 flex-wrap">
                {rayons.map((r) => (
                  <button
                    ...
                    style={{
                      backgroundColor: rayon === r ? "#1A2562" : "white",
                      color: rayon === r ? "white" : "#1A2562",
              ...
              <div className="flex gap-1.5">
                {(["Tous", "Demande", "Offre"] as const).map((t) => (
                  <button
                    ...
                    className="px-3 py-1 rounded-full text-xs font-semibold transition-colors"
                    style={{
                      backgroundColor: typeFiltre === t ? "#1A2562" : "white",
                      color: typeFiltre === t ? "white" : "#1A2562",
                      border: "1px solid #c8b88a",
                    }}
                  >
                    {t}
            ...
            <label className="flex items-center gap-2.5 cursor-pointer select-none">
              <div
                onClick={() => setTrocSeulement(!trocSeulement)}
                className="w-10 h-5 rounded-full relative transition-colors"
                style={{ backgroundColor: trocSeulement ? "#2D6A4F" : "#c8b88a" }}
              >
                <div
                  className="absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform"
                  style={{ transform: trocSeulement ? "translateX(22px)" : "translateX(2px)" }}
                />
              </div>
              <span className="text-sm font-semibold" style={{ color: "#5a4a30" }}>
                🔄 Accepte troc ou service en contrepartie
              </span>
            </label>
          </div>
          ...
          {annoncesFiltrees.length === 0 ? (
            <div className="rounded-xl p-8 text-center" style={{ backgroundColor: "white", border: "1px solid var(--cream-dark)" }}>
              <p className="opacity-50 text-sm">Aucune annonce dans ce périmètre avec ces filtres.</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {annoncesFiltrees.map((a) => (
                <div key={a.id} className="rounded-xl p-5 flex flex-col gap-3"
                  style={{ backgroundColor: "white", border: "1px solid var(--cream-dark)", boxShadow: "0 2px 10px rgba(0,0,0,0.05)" }}>
                  ...
                  <h4 className="font-bold text-sm leading-snug" style={{ color: "var(--navy)" }}>{a.titre}</h4>
                  ...
                  <div className="flex items-center justify-between mt-auto pt-2" style={{ borderTop: "1px solid var(--cream-dark)" }}>
                    <span className="text-xs opacity-50">📍 {a.ville} · à {a.dist} km</span>
                    <button className="px-4 py-1.5 rounded-full text-xs font-bold hover:opacity-80"
      ...
      {!deptActif && (
        <p className="text-center opacity-40 text-sm pb-4" style={{ color: "var(--navy)" }}>
